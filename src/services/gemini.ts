...
export async function transcriptionAudio(
  ...
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: "Transcreva o audio para português. Seja preciso e natural na transcrição. Mantenha a pontuação e a formatação. Divida o texto em parágrafos quando for apropriado.",
      },
      {
        inlineData: {
          mimeType,
...
export async function generateAnswer(
  ...
  if (!response.text) {
    throw new Error("Não foi possível gerar a resposta Gemini");
  }
  return response.text;
}
